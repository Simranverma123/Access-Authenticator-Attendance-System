...
SERVICE_ACCOUNT_FILE = 'C:/Users/KIIT/Desktop/Access-Authenticator-main/access-authenticator-385111-93691effbea6.json'
...
with open('C:/Users/KIIT/Desktop/Access-Authenticator-main/authorized.txt', 'r') as f:
    ...
    while attempts < 3:
        ...
        if password == "admin":
            ...
            messagebox.showerror("Incorrect Password",
                                 f"Sorry, the password is incorrect. You have {3 - attempts} attempts left.")

    if attempts == 3:
        messagebox.showerror("Too Many Attempts",
                             "You have entered an incorrect password more than three times. Your photo will be taken and stored.")
...
def register_roll_number():
    ...
    if roll_number:
        ...
        with open('C:/Users/KIIT/Desktop/Access-Authenticator-main/authorized.txt', 'r') as file:
            content = file.readlines()

        # Check if the roll number is already present in the file
        if roll_number + '\n' in content:
            messagebox.showerror("Roll Number Already Registered",
                                 f"The roll number {roll_number} is already registered.")
        else:
            # Add the roll number to the file if it's unique
            with open("C:/Users/KIIT/Desktop/Access-Authenticator-main/authorized.txt", "a") as f:
                f.write(f"{roll_number}\n")
            messagebox.showinfo("Roll Number Registered",
                                f"The roll number {roll_number} has been registered successfully.")
...
def delete_roll_number():
    ...
    if roll_number is not None:
        ...
        with open('C:/Users/KIIT/Desktop/Access-Authenticator-main/authorized.txt', 'w') as file:
...
register_roll_button_admin = tk.Button(admin_window, text="REGISTER ROLL NUMBER", width=50, height=10, bg="white",
                                       command=register_roll_number)
register_roll_button_admin.pack()

# Add a button to the admin access to delete a roll number
delete_roll_button_admin = tk.Button(admin_window, text="DELETE ROLL NUMBER", width=50, height=10, bg="white",
                                     command=delete_roll_number)
